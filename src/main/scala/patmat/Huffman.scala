...
object Huffman {
   ...
   */
  def until(xxx:  List[CodeTree] => Boolean, yyy: List[CodeTree] => List[CodeTree]):List[CodeTree] => List[CodeTree] = list => {
    if(xxx(list)) list else until(xxx, yyy)(yyy(list))
  }
  ...
  def encode(tree: CodeTree): List[Char] => List[Bit] = { 
	...
	case l: Leaf => encode0(tree, xs, accu)
     ...
     (c: List[Char]) => encode0(tree, c, List())
  }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =  table match {
    case Nil => println("CAN'T ENCODE CHARCTER " + char); List()
...
def main(args : Array[String]) : Unit = {
    val toto = "HUFMAN CODING IN SCALA done by mohan "
